...
The level timer is **wall-clock based**: each frame advances an accumulated `elapsed_ms` by the real time since the previous frame, and any time spent in a blocking sub-loop (pause menu, cheat menu, READY! overlay) is excluded so it never counts against the clock. On level start and after each respawn a "READY!" banner is held over the frozen scene for ~1 second (`SpritesMixin.render_text`), and the event queue is flushed afterwards so buffered keypresses cannot leak across resets. Player input is taken from `KEYDOWN` events only (a single source of truth), and a reversal of direction is applied immediately, so a mid-cell turn never overshoots to the wrong cell.
...
core/
  parser.py           ConfigParser class — schema-driven JSON-with-comments
                      parsing, validation, clamping, defaults
...
display/
  ...
  menu.py             MenuNav — shared keyboard/mouse vertical-menu driver
  menu_screens_mixin.py     Pre-game screens (arcade menu, highscores,
                            instructions)
  overlay_screens_mixin.py  In-game overlays (pause, cheat, jumpscare
                            screamer, end / win screen)
  sprite_mixin.py     Sprite loading, scaling, animation frames, text rendering
...
- `PygameViewer` is composed from five mixins (`RendererMixin`, `HudMixin`, `MenuScreensMixin`, `OverlayScreensMixin`, `SpritesMixin`) to keep each concern in its own file while sharing a single `self.screen`.
...
03/06/2026:
	mlorenzo:
		...
		- Rewrote the parser as a schema-driven `ConfigParser` class
		- Added the missing docstrings and a few clarifying comments (A* pathfinding, _build_monitor, cheat menu, ...)
		- Refactored the longest files for readability:
